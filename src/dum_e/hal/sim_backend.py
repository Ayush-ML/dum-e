...
class SimBackend(Actuator):
    ...
    def __init__(self) -> None:
        ...
        self._gravity_z = config["gravity_z"]
        self._max_force = config["max_joint_force"]
        self._use_gui = config["use_gui"]
    ...
    def emergency_stop(self) -> None:
        ...
        for joint_index in self._joint_indices:
            pb.setJointMotorControl2(bodyUniqueId=self._robot_id, jointIndex=joint_index, controlType=pb.VELOCITY_CONTROL, force=self._max_force, targetVelocity=0.0)
    ...
    def set_joint_targets(self, targets: list[float]) -> None:
        ...
        for joint_index, target in zip(self._joint_indices, targets):
            pb.setJointMotorControl2(bodyUniqueId=self._robot_id, jointIndex=joint_index, controlType=pb.POSITION_CONTROL, targetPosition=target, force=self._max_force)